...
export default function PublishPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const demoSlug = "alpine-retreat"; // hardcoded for demo mapping (params.id could be new-draft)
    const fullLink = `stayos.app/g/${demoSlug}`;
                    ...
                    <Card className="border-slate-200 shadow-sm border-2">
                        <CardContent className="p-6 flex flex-col items-center text-center space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-mono text-blue-600 mt-1 bg-blue-50 py-1 px-3 rounded-md">{fullLink}</p>
                            </div>
                            <div className="flex gap-2 w-full pt-2">
                                <Button
                                    variant="outline"
                                    className="flex-1 gap-2"
                                    onClick={() => copyToClipboard(`https://${fullLink}`, setCopiedLink)}
                                >
                                    {copiedLink ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
                                    {copiedLink ? "Copied!" : "Copy Link"}
                                </Button>
                                <Link href={`/g/${demoSlug}`} target="_blank" className="flex-1">
