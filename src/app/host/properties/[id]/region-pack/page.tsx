...
export default function RegionPackReviewPage({ params }: { params: { id: string } }) {
    ...
    const [places, setPlaces] = useState([
        { id: 1, cat: "food", title: "Bakery du Village", dist: "5 min walk", status: "pin", notes: "Great morning bakery" },
        { id: 2, cat: "food", title: "Le Caveau", dist: "10 min walk", status: "approve", notes: "Best fondue in town" },
        { id: 3, cat: "food", title: "Tourist Trap Café", dist: "2 min walk", status: "hide", notes: "Overpriced" },
        { id: 4, cat: "ski", title: "Alpine Rentals", dist: "15 min walk", status: "approve", notes: "Partner shop, 10% off" },
    ]);
    ...
    const toggleListening = () => {
        if (!isListening) {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    setPlaces(p => p.map(pl => pl.id === 1 ? { ...pl, status: "pin" } : pl));
    ...
    const handleStatus = (id: number, status: string) => {
        setPlaces(p => p.map(pl => pl.id === id ? { ...pl, status } : pl));
    };
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 p-8 overflow-y-auto relative">
                    <div className="max-w-4xl mx-auto space-y-4">
                            ...
                            <Card key={p.id} className={`border transition-all duration-300 ${p.status === 'hide' ? 'opacity-50 border-slate-200 bg-slate-50' :
                                ...
                                <CardContent className="p-6 flex items-center justify-between">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-slate-500 mt-1">{p.dist} — {p.notes}</p>
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            className={`h-10 w-10 shrink-0 rounded-full ${p.status === 'hide' ? 'bg-slate-800' : ''}`}
                                        >
                                            <X className="w-4 h-4" />
                                        </Button>
                                    </div>
                        ...
                        {places.filter(p => p.cat === activeTab).length === 0 && (
